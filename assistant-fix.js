(() => {
  ...
  const add = (text, cls) => { const d=document.createElement('div'); d.className='msg '+cls; d.textContent=text; log.appendChild(d); log.scrollTop=log.scrollHeight; };
  const answer = (raw) => {
    const m=raw.toLowerCase().trim();
    if (/what (do|can) you have for me|what have you for me|what can you offer|what do you offer|tell me about shakti|about shakti|what is shakti millets|who is shakti millets/.test(m)) return '🌾 Welcome to Shakti Millets!\n\nShakti Millets brings you quality millets and traditional grains for everyday meals as well as business requirements. Our catalogue includes Pearl Millet (Kambu), Kodo Millet, Barnyard Millet, Little Millet, Foxtail Millet, Browntop Millet, Proso Millet, Ragi, Sorghum, Horsegram and traditional red-rice varieties.\n\nWe offer retail packs, a 5 Little Millets Combo, and B2B/bulk supply support for restaurants, hotels, cafés, cloud kitchens, retailers, supermarkets, food manufacturers and distributors.\n\nI can help you choose a product, explain its uses and nutrition, compare products, suggest recipes, share published MRP and guide you with bulk enquiries. What would you like to explore?';
    if (/^(hi|hello|hey|namaste|good morning|good afternoon|good evening)\b/.test(m)) return 'Hello! 👋 Welcome to Shakti Millets. I can tell you about our products, millet nutrition, cooking ideas, recipes, prices and B2B supply. What would you like to know?';
  ...
  const send=()=>{const text=input.value.trim();if(!text)return;add(text,'msg-user');input.value='';button.disabled=true;button.textContent='...';setTimeout(()=>{add(answer(text),'msg-bot');button.disabled=false;button.textContent='Send';},180);};
  button.addEventListener('click',send); input.addEventListener('keydown',e=>{if(e.key==='Enter')send();});
})();
